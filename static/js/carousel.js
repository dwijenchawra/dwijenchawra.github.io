document.addEventListener('DOMContentLoaded', function() {
  ...
  carouselContainers.forEach(function( carouselContainer ) {
      ...
      const bullets = carousel.querySelectorAll('.bullets li');
      const descriptions = carouselContainer.querySelectorAll('.descriptions li');
      ...
      if(amountvisible>1) {
        var removeels = carousel.querySelectorAll('.bullets li:nth-last-child(-n + '+(amountvisible-1)+')');
        removeels.forEach(function(removeel) {
          removeel.remove();
        });
        removeels = carousel.querySelectorAll('.descriptions li:nth-last-child(-n + '+(amountvisible-1)+')');
        removeels.forEach(function(removeel) {
          removeel.remove();
        });
      }
      ...
      const setSelected = function() {
          ...
          carousel.querySelector('.bullets li:nth-child('+nthchild+')').classList.add('selected');
          carouselContainer.querySelector('.descriptions li:nth-child('+nthchild+')').classList.add('selected');
      ...
      const nextSlide = function() {
          if(!carousel.querySelector('.bullets li:last-child').classList.contains('selected')) {
              carousel.querySelector('.bullets li.selected').nextElementSibling.querySelector('a').click();
          } else {
              carousel.querySelector('.bullets li:first-child a').click();
          }
      }

      const prevSlide = function() {
          if(!carousel.querySelector('.bullets li:first-child').classList.contains('selected')) {
              carousel.querySelector('.bullets li.selected').previousElementSibling.querySelector('a').click();
          } else {
              carousel.querySelector('.bullets li:last-child a').click();
          }
      }
